...
~~**Subscription limit race condition**~~ — **RESOLVED** 2026-04-22: `server/src/routes/ai.ts` uses two-level check: fast non-atomic early exit (saves wasted API call), plus atomic transactional re-check inside `$transaction` at message persist time. Concurrent bypass is blocked at the transaction level.
