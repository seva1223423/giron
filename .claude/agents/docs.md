...
- **Test count drifts** → `tests` agent added a suite; `docs` agent updates the suite count in CLAUDE.md.
  - Server: `cd server && npx jest --no-coverage --forceExit 2>&1 | tail -5` (expected: 19 suites, ~532 tests)
  - Client: `npm test -- --no-coverage --forceExit 2>&1 | tail -5` (expected: 29 suites, ~542 tests)
