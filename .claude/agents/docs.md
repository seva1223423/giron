...
- **Test count drifts** → `tests` agent added a suite; `docs` agent updates the suite count in CLAUDE.md.
  - Server: `cd server && npx jest --no-coverage --forceExit 2>&1 | tail -5` (expected: 16 suites, ~339 tests)
  - Client: `npm test -- --no-coverage --forceExit 2>&1 | tail -5` (expected: 25 suites, ~448 tests)
