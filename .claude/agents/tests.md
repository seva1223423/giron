...
```
server/src/__tests__/           — server integration tests (Jest + Supertest, 16 suites, 339 tests)
...
src/__tests__/                  — client store unit tests (Jest, 25 suites, 448 tests)
...
**Expected baseline:** 339 server tests pass (16 suites), 448 client tests pass (25 suites).
...
- **Test count reference** — as of 2026-04-22: 339 server tests (16 suites), 448 client tests (25 suites). Before adding a new test suite, confirm the file doesn't already exist in the relevant `__tests__/` directory.
