...
```
server/src/__tests__/           — server integration tests (Jest + Supertest, 19 suites, 532 tests)
...
src/__tests__/                  — client store unit tests (Jest, 29 suites, 542 tests)
...
**Expected baseline:** 532 server tests pass (19 suites), 542 client tests pass (29 suites).
...
- **Test count reference** — as of 2026-04-22: 532 server tests (19 suites), 542 client tests (29 suites). Before adding a new test suite, confirm the file doesn't already exist in the relevant `__tests__/` directory.
