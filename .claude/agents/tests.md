...
```
server/src/__tests__/           — server integration tests (Jest + Supertest, 19 suites, 510 tests)
  ...
  validation.test.ts            — Zod schema boundary tests: meal, weight, registration, workout, program, routines, cardio, body measurements, sleep/bedtime, profile update, nutrition targets (134 tests)
  setup.ts                      — JWT secrets + env vars (runs before every test)
  __mocks__/
    expo-server-sdk.ts          — mock for push notification SDK

src/__tests__/                  — client store unit tests (Jest, 29 suites, 512 tests)
...
**Expected baseline:** 510 server tests pass (19 suites), 512 client tests pass (29 suites).
...
- **Test count reference** — as of 2026-04-22: 510 server tests (19 suites), 512 client tests (29 suites). Before adding a new test suite, confirm the file doesn't already exist in the relevant `__tests__/` directory.
