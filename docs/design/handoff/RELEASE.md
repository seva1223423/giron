# Iron Gym — релиз-пакет (всё, что сделано)
...
Готовые файлы, которые мы правили в существующем приложении iron-gym. Лежат
...
**Как применить:** скопируйте `app-patches/src/` поверх `iron-gym/src/`. Старые
