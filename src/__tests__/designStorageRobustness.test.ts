...
describe('getStorageUsage', () => {
  ...
  test('under 3MB returns ok', async () => {
    ...
    await AsyncStorage.setItem('iron-gym-workouts', smallPayload);
    const usage = await getStorageUsage();
    expect(usage.warningLevel).toBe('ok');
  });
  ...
  test('between 3MB and 5MB returns warning', async () => {
    ...
    await AsyncStorage.setItem('iron-gym-workouts', bigPayload);
    const usage = await getStorageUsage();
  ...
  test('over 5MB returns critical', async () => {
    ...
    await AsyncStorage.setItem('iron-gym-workouts', hugePayload);
    const usage = await getStorageUsage();
    expect(usage.warningLevel).toBe('critical');
  });

  test('breakdown sorted descending', async () => {
    await AsyncStorage.setItem('iron-gym-workouts', 'x'.repeat(100));
    await AsyncStorage.setItem('iron-gym-nutrition', 'x'.repeat(500));
    const usage = await getStorageUsage();
  ...
  test('mb value is correctly formatted with 2 decimal places', async () => {
    await AsyncStorage.setItem('iron-gym-workouts', 'x'.repeat(5000));
    const usage = await getStorageUsage();
  ...
  test('unicode strings counted correctly (UTF-16 approximation)', async () => {
    await AsyncStorage.setItem('iron-gym-auth', 'Привет мир'.repeat(100));
    const usage = await getStorageUsage();
  ...
  test('bytes = length * 2 matches formula', async () => {
    ...
    await AsyncStorage.setItem('iron-gym-workouts', payload);
    const usage = await getStorageUsage();
    const workouts = usage.breakdown.find((b) => b.key === 'iron-gym-workouts');
...
describe('Persisted data parse failures', () => {
  ...
  test('invalid JSON stored value round-trips', async () => {
    await AsyncStorage.setItem('iron-gym-workouts', '{not-json');
    const raw = await AsyncStorage.getItem('iron-gym-workouts');
    expect(raw).toBe('{not-json');
  });

  test('null-string stored value returns "null"', async () => {
    await AsyncStorage.setItem('iron-gym-workouts', 'null');
    const raw = await AsyncStorage.getItem('iron-gym-workouts');
  ...
  test('empty object "{}" parses safely', async () => {
    await AsyncStorage.setItem('iron-gym-workouts', '{}');
    const raw = await AsyncStorage.getItem('iron-gym-workouts');
  ...
  test('deep-nested object parses without stack blowing', async () => {
    ...
    await AsyncStorage.setItem('iron-gym-workouts', deep);
    const raw = await AsyncStorage.getItem('iron-gym-workouts');
    expect(() => JSON.parse(raw!)).not.toThrow();
  ...
  test('64KB payload round-trips intact', async () => {
    ...
    await AsyncStorage.setItem('iron-gym-workouts', big);
    const raw = await AsyncStorage.getItem('iron-gym-workouts');
  ...
  test('unicode characters round-trip intact', async () => {
    ...
    await AsyncStorage.setItem('iron-gym-workouts', s);
    const raw = await AsyncStorage.getItem('iron-gym-workouts');
    expect(raw).toBe(s);
  });
...
describe('Store keys are unique and stable', () => {
  ...
  test('iron-gym prefix for all store keys (except cardio legacy)', () => {
    const prefixed = KEYS.filter((k) => k.startsWith('iron-gym-'));
    const unprefixed = KEYS.filter((k) => !k.startsWith('iron-gym-'));
