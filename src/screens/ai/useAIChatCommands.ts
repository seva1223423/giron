 ...
 *
 * Wires `parseChatCommand` to live Zustand stores + a few async server
 * services. Returns `tryHandle(text)` → true if a command was recognized
 * AND acted upon. Caller (AIChatScreen) uses the boolean to short-circuit
 * the server send.
 *
 * Async pattern: handlers that hit the network (activate program, log
 * body weight, add recipe) are fire-and-forget Promises wrapped in
 * `void asyncFn()`. `tryHandle` itself stays synchronous so the chat
 * input handler doesn't have to await.
 */
...
/**
 * Exported for unit testing — `executeCommand` is the pure dispatch
 * surface, easier to test without a React render shell. Async handlers
 * are dispatched with `void` so the dispatcher signature stays sync.
 */
export function executeCommand(cmd: ParsedCommand): void {
  switch (cmd.type) {
    ...
    // Phase F — async fire-and-forget
    case 'activate_program':    return void handleActivateProgram(cmd.name);
    case 'log_body_weight':     return void handleLogBodyWeight(cmd.kg);
    case 'swap_exercise':       return handleSwapExercise(cmd.fromName, cmd.toName);
    case 'add_recipe':          return void handleAddRecipe(cmd.name);
  }
}
...
function handleResetWater(): void {
  ...
  if (current <= 0) { toast.info('Вода уже 0'); return; }
...
async function handleActivateProgram(name: string): Promise<void> {
  const programs = useWorkoutStore.getState().programs ?? [];
  const match = findByName(programs, name);
  if (!match) {
    const top3 = programs.slice(0, 3).map((p) => p.name).join(', ');
    toast.warn(top3 ? `Не найдено. Есть: ${top3}` : 'Программ нет');
    return;
  }
  try {
    await useWorkoutStore.getState().updateProgram(match.id, { isActive: true });
    toast.success(`Программа: ${match.name}`);
  } catch {
    toast.error('Не удалось активировать программу');
  }
}

async function handleLogBodyWeight(kg: number): Promise<void> {
  try {
    await userService.addWeight(kg);
    // Fire-and-forget profile sync so ProfileScreen's weight history
    // picks up the new entry on its next read. Errors swallowed — the
    // primary POST already succeeded.
    useAuthStore.getState().fetchProfile().catch(() => {});
    toast.success(`Вес тела: ${kg} кг`);
  } catch {
    toast.error('Не удалось сохранить вес');
  }
}

function handleSwapExercise(fromName: string, toName: string): void {
  const state = useWorkoutStore.getState();
  const aw = state.activeWorkout;
  if (!aw) { toast.warn('Нет активной тренировки'); return; }
  const exIdx = aw.currentExerciseIndex;
  const current = aw.workout.exercises[exIdx]?.exercise;
  const currentName = (current?.name ?? '').toLowerCase();
  const fromQuery = fromName.toLowerCase();
  // Sanity check: the "from" name should match the current exercise,
  // otherwise the user probably mistyped (e.g. wanted to swap a different
  // exercise but is on jim now).
  if (!currentName.includes(fromQuery) && !fromQuery.includes(currentName.slice(0, 5))) {
    toast.warn(`Текущее упражнение не "${fromName}"`);
    return;
  }
  const match = findByName(EXERCISES, toName);
  if (!match) {
    toast.warn(`Упражнение "${toName}" не найдено`);
    return;
  }
  const ok = state.replaceExerciseInWorkout(exIdx, match);
  if (ok) toast.success(match.name);
  else toast.warn('Не удалось заменить упражнение');
}

/**
 * Pick a meal type based on local hour. Crude but reasonable for "съел X"
 * — the user gets one tap to fix it in the meal log if needed.
 */
...
async function handleAddRecipe(name: string): Promise<void> {
  const recipes = useRecipesStore.getState();
  const all = [...(recipes.curated ?? []), ...(recipes.mine ?? [])];
  const match = findByName(all, name);
  if (!match) {
    toast.warn(`Рецепт "${name}" не найден`);
    return;
  }
  const today = localDateStr(new Date());
  try {
    await recipeService.addToDiary(match.id, {
      date: today,
      mealType: inferMealType(),
      servings: 1,
    });
    // Server creates the Meal record and expands Recipe.ingredients
    // into Meal.items[] with scaled macros. We just resync from server
    // to pick up the new meal in the local nutrition store.
    useNutritionStore.getState().syncMealsFromServer(today).catch(() => {});
